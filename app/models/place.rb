...
class Place < ApplicationRecord
  ...
  def self.ransackable_attributes(auth_object = nil)
    ["id", "address", "created_at", "description", "name", "toilet", "updated_at", "vendingmachine", "schoolgrade_id"]
  end
  ...
  def self.sort_by_favorite_count(direction)
    includes(:favorites).
      group(:id).
      order(Arel.sql("(SELECT COUNT(favorites.id) FROM favorites WHERE favorites.place_id = places.id) #{direction}"))
  end
end
